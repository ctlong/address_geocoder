...
describe AddressGeocoder do
  describe 'when initialized' do
    it "should throw an error if the country is not recognized" do
      expect{ AddressGeocoder::Base.new(country: 'United States', city: 'Phoenix') }.to raise_error
    end
    it "should have attr accessors" do
      address_geocoder = AddressGeocoder::Base.new(api_key: '12345', country: 'US', state: 'CO', street: '301 First St.', city: 'Crested Butte', postal_code: '10022')
    ...
    it "should not have called google" do
      address_geocoder = AddressGeocoder::Base.new(country: 'US', city: 'Phoenix')
      expect(address_geocoder.google_response).to eq(nil)
    end
  end
  ...
  describe '#valid_address?' do
    context 'when address can be not recognized' do
      it 'returns false' do
        # when only city
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'CN', city: 'Tokyo')
        expect(address_geocoder.valid_address?).to eq(false)
        sleep(1)
        # when only postal code
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'BR', postal_code: 'A6000A')
        expect(address_geocoder.valid_address?).to eq(false)
        sleep(1)
        # when only state
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'JP', state: 'Ohio')
        expect(address_geocoder.valid_address?).to eq(false)
        sleep(1)
        # when only street
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'CH', street: '10, On Lok Mun Street')
        expect(address_geocoder.valid_address?).to eq(false)
        sleep(1)
        # when city vs postal code
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'CN', city: 'Tokyo', postal_code: '102600')
        expect(address_geocoder.valid_address?).to eq(false)
        sleep(1)
        # when city vs state
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'CN', city: 'Tokyo', state: 'Liaoning')
        expect(address_geocoder.valid_address?).to eq(false)
      end
    ...
    context "when address can be recogized" do
      it "returns true" do
        # when only city
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'CN', city: 'Beijing')
        expect(address_geocoder.valid_address?).to eq(true)
        sleep(1)
        # when only postal code
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'BR', postal_code: '01501-000')
        expect(address_geocoder.valid_address?).to eq(true)
        sleep(1)
        # when only state
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'JP', state: 'Saitama')
        expect(address_geocoder.valid_address?).to eq(true)
        sleep(1)
        # when only street
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'CH', street: 'Brunngasshalde')
        expect(address_geocoder.valid_address?).to eq(true)
        sleep(1)
        # when city vs postal code
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'CN', city: 'Beijing', postal_code: '100050')
        expect(address_geocoder.valid_address?).to eq(true)
        sleep(1)
        # when city vs state
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'BR', city: 'Belo Horizonte', state: 'Minas Gerais')
        expect(address_geocoder.valid_address?).to eq(true)
        sleep(1)
        ...
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'GR', state: 'Eastern Macedonia and Thrace', postal_code: '671 00')
        expect(address_geocoder.valid_address?).to eq(true)
        sleep(1)
        # when street, city, state
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'CI', street: 'Boulevard Houphouët-Boigny', city: 'San-Pédro', state: 'Bas-Sassandra')
        expect(address_geocoder.valid_address?).to eq(true)
        sleep(1)
        # when street, city, postal_code
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'FR', street: '8 Boulevard Léon Bureau', city: 'Nantes', postal_code: '44200')
        expect(address_geocoder.valid_address?).to eq(true)
      end
    end
  end
  ...
  describe "#suggested_address" do
    context "when address can be not recogized" do
      it "returns false" do
        ...
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'CN', city: 'Hangzhou', state: 'Osaka Prefecture')
        expect(address_geocoder.valid_address?).to eq(false)
        expect(address_geocoder.suggested_addresses).to eq(false)
        sleep(1)
      end
    ...
    context "when address is not valid, but can still be recognized" do
      it "returns a hash with keys: country state city postal_code street" do
        ...
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'CN', city: 'Beijing', postal_code: 'AE102600')
        expect(address_geocoder.valid_address?).to eq(false)
    ...
    context "when address can be recogized" do
      it "returns a hash with keys: country state city postal_code street" do
        # when only city
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'US', city: 'Seattle')
        ...
        sleep(1)
        # when postal code vs street
        address_geocoder = AddressGeocoder::Base.new(api_key: ENV['AddressGeocoderApiKey'], country: 'US', postal_code: '10022', street: '475 Madison Ave')
